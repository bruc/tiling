...
d_path=$(readlink -f ../data/2021-scAAV-CBA-eGFP/1-demux)

declare -a fa_s
declare -a names

barcodes=bc1001:bc1002:bc1003:bc1008:bc1009:bc1010:bc1011:bc1012

templates=scGFP:scGFP:scGFP:scGFP:scGFP:scGFP:scGFP:scGFP

fa_s=($(readlink -f ../references/scAAV_CBA_eGFP_split.fasta))
...
tiling_d="../tiling/2021-scAAV-CBA-eGFP"
if [[ ! -d $tiling_d ]]
then
...
for ((i=0; i<n_names; i++))
do
    makeblastdb -in ${fa_s[i]} -dbtype nucl
    execute_sql "
...
do
    fa=${d_path}/demux.${barcode}--${barcode}.fasta
    if [[ ! -r $fa ]]
    then
